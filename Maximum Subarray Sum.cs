...
class Result
{
    ...
    for (int i = 0; i < a.Count; i++)
    {
        ...
        // Look for the smallest prefix sum that is larger than current sum
        var nextLarger = prefixSums.GetViewBetween(currentSum + 1, long.MaxValue).FirstOrDefault();
        if (nextLarger != default(long))
        {
            maxSum = Math.Max(maxSum, (currentSum - nextLarger + m) % m);
        }
